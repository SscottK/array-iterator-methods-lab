...
veryOldInventors = inventors.filter((inventor) => {
    return parseInt(inventor.year) > 1500 && parseInt(inventor.year) < 1599
})
...
totalYearsLived = inventors.map((inventor) => {
    return inventor.passed- inventor.year
}).reduce((runningTotal, age) => {
...
travelMethodCounts = travelMethods.reduce((runningTotal, methods) => {
   if (runningTotal[methods]) {
    runningTotal[methods] = runningTotal[methods] + 1
   } else {
    runningTotal[methods] = 1
   }
   return runningTotal   
